...
| **M7** (Active, v0.6.0 target) | Usable Workflows + Observability: workflow data-flow output/input bindings, execution log/event streaming, OTEL + Uptrace (traces/metrics/logs/APM + login UI, compose & Helm), context propagation (trace/data/correlation), git MCP shim over git-adapter, expert-agent substrate + agents/examples, reusable templates + first real runnable workflows, first-run UX / zero-secret Ollama quickstart (clone → one command → result, EPIC #1370), quality/supply-chain fixes, test rigor (benchmarks/fuzz/correlation), authoring & observability docs | M-dx developer-experience program (zero-Temporal Day-0 engine #1359), M8 CNCF submission |
