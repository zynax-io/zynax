...
`zynax-ci images` manages container image references across the repository, using
`images/images.yaml` as the single source of truth.

### `zynax-ci images sync`

Stamps every **banner-marked region** in workflow files and Dockerfiles with the canonical
digests declared in `images/images.yaml`.

Banner format (region delimited by these two comment lines):
```
...
Any content between the two banner lines is replaced on each `sync` run.
Do **not** hand-edit banner-marked regions — your changes will be overwritten.

```bash
zynax-ci images sync   # or: make sync-images
```

### `zynax-ci images check`

Verifies that every banner-marked region in the repository matches the current contents of
`images/images.yaml`. Exits 1 if any region is stale. Used as a CI gate.

```bash
zynax-ci images check  # or: make check-images
```
