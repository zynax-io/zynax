...
// workflow publish) — they all submit a file or scenario dir.
const publishUse = "publish <file|scenario-dir>"

// publishCmd is a thin, top-level alias over `apply`. It submits a Workflow YAML
// manifest to the api-gateway; the manifest kind is auto-detected on the apply
// path exactly as `zynax apply` does, so `publish` adds no behaviour of its own —
// it only gives the quickstart a noun-free "ship it" verb (canvas O20).
var publishCmd = &cobra.Command{
	Use:     publishUse,
	Short:   "Publish a Workflow manifest — alias for apply",
	Long:    "Publish a Workflow YAML manifest to the api-gateway. The manifest kind is\nauto-detected, exactly like `zynax apply`. This is a thin alias kept for the\nquickstart's noun-first mental model; `apply` remains available unchanged.\n\nAgent identity is not published here — apply a zynax.io/v1alpha1 Agent custom\nresource with kubectl (ADR-039, docs/patterns/agent-crd-migration.md).",
