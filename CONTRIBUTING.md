...
> **Image digests are managed via `images/images.yaml`** — do not hand-edit any region
> delimited by `# BEGIN generated by zynax-ci images sync` / `# END generated by zynax-ci
> images sync` banners in workflow files or Dockerfiles. Run `make sync-images` to update
> them and `make check-images` to verify they match.
