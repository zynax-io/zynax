...
| M6 — K8s Production | ✅ **Complete** | v0.5.0 | **Delivered:** mTLS (ADR-020 #464), cosign+SBOM+multi-arch (ADR-025 #465), Postgres-backed task-broker + agent-registry (#626), Helm charts for all 7 services (#765), EventBus over NATS JetStream (#772), `images.yaml` source-of-truth + drift gate (ADR-024 #855), orchestrator self-hosting + concurrency hardening (#873, #1001), memory-service KV + vector (#773). ArgoEngine + multi-engine dispatch (#766, #798), multi-namespace (#767), policy/rate-limit (#768), Prometheus /metrics (#491), SDK PyPI (#769), e2e harness (#770), native multi-arch build (#837), gRPC health protocol (#74 #656), Postgres off Bitnami → Docker Official `postgres:17` (ADR-026 #1073), DevAuto Wave 4 self-hosted issue-delivery manifests delivered to the platform-readiness boundary (#881 — O8 e2e → M7 #1103), e2e-green gate (#1086), CI-E2E gate (#771). Released v0.5.0 2026-06-12. |
| M7 — Usable Workflows + Observability | ✅ **Complete** | v0.7.0 (joint with M8, released 2026-07-10) | Usable Workflows + Observability: workflow data-flow output/input bindings (#1167), execution log/event streaming (#468), OTEL + Uptrace traces/metrics/logs/APM (#467), context propagation (#1168), git MCP shim (#1169), expert-agent substrate (#1170), reusable templates + real workflows (#1171), quality/supply-chain fixes (#1172), authoring & observability docs (#1173), test rigor (#469). See `docs/milestones/M7-planning.md`. |
| M8 — CNCF Sandbox + thin-Zynax reduction | ✅ **Complete** | v0.7.0 (joint with M7) | Governance docs (#494 #495), CRD-native scheduler (ADR-039 #1571), Compose-runtime removal (ADR-041 #1572), thin Workflow CRD front-end (ADR-043 #1573), Envoy Gateway edge auth/rate-limit (ADR-044 #1574), ValidatingAdmissionPolicy allow-list (ADR-045 #1575), direct NATS JetStream + facade deprecation (ADR-046 #1576), GitHub merge queue (ADR-047 #1680). CNCF filing is a maintainer action (`docs/cncf/sandbox-submission.md`). |
| M9 — Hard Removals + Conformance | 🚧 **Active** | v0.8.0 (target) | Agent-registry push-path hard-removal (ADR-039, epic #1674), EventBusService facade hard-removal (ADR-046, epic #1675), named engine-conformance suite (epic #1692). See `docs/milestones/M9-planning.md`. |

See `ROADMAP.md` for the narrative roadmap and `docs/milestones/M9-planning.md` for active execution details.
...
| event-bus | ~883 | ~1,401 | ✅ Implemented |
| memory-service | ~910 | ~1,342 | ✅ Implemented |
