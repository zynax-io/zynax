...
class AdapterSettings(BaseSettings):
    ...
    Attributes:
        ...
        adapter_endpoint: gRPC address the task-broker dials for this adapter
            (``ADAPTER_ENDPOINT``), e.g. ``"llm-adapter:50057"``.
        registry_addr: agent-registry gRPC address (``REGISTRY_ADDR``).
        grpc_port: Port the adapter's own gRPC server binds to
            (``ZYNAX_LLM_ADAPTER_GRPC_PORT``, default ``50057``).
    """
    ...
    grpc_port: int = Field(default=50057, alias="ZYNAX_LLM_ADAPTER_GRPC_PORT", ge=1)
...
def build_agent_def(
    ...
    """Build an ``AgentDef`` proto from adapter settings and capability list.

    Args:
        settings: Adapter settings containing ``agent_id`` and ``adapter_endpoint``.
        capability_names: Ordered list of capability names to register.
