...
**Container image refs (ADR-024):** `images/images.yaml` is the single source of truth for
all container image digests and tags used across workflow files and Dockerfiles. Any region
delimited by `# BEGIN generated by zynax-ci images sync` / `# END generated by zynax-ci
images sync` banners is **generated** — do not hand-edit. Run `make sync-images` to update
and `make check-images` to verify. CI fails if any banner-marked region is stale.
To bump a digest: (1) edit **only** `images/images.yaml`; (2) run `make sync-images` to stamp
every consumer; (3) open one PR — no need to know which downstream files exist. Register a new
consumer by adding it to the `consumers:` list in `images.yaml`, never by wiring it in by hand.
Intentionally **not** managed here (edit directly): Docker Compose `:main` dev tags, Helm
`values.yaml` image fields, and the README install table. Full rationale: ADR-024.
