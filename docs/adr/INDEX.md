...
| [ADR-044](ADR-044-gateway-api-edge-auth-ratelimit.md) | Edge delegation — gateway bearer-auth + rate-limit to a Gateway API edge (Envoy Gateway) | Accepted | 2026-07-03 | api-gateway (delete `auth.go`/`ratelimit.go` in-process middleware; NetworkPolicy edge-lock), `helm/` Gateway API + Envoy Gateway edge (`SecurityPolicy` auth, global `BackendTrafficPolicy` rate-limit), `scripts/e2e/cluster-up.sh`, cli `--api-key` — amends ADR-040 — M8 EPIC F (#1574) |
