...
| 3 ✅ | [#1774](https://github.com/zynax-io/zynax/issues/1774) | machine-readable per-engine matrix artifact + one-command local run — **done**: every e2e run emits `zecs-matrix.json` (per-leg outcome recording → `ZECS matrix` fan-in job, 90-day retention), covering every engine the engine-adapter can be configured with. Only an observed success renders `PASS`; a leg that never ran is present as `NOT_RUN`, and `not_in_leg` (by design) stays distinct from `not_executed` (which forces `INCOMPLETE`). Per-leg `enforcement` + a separate `enforced_result` keep the advisory argo leg from reading as merge-blocking (G6). `make conformance-matrix ENGINE=<engine>` reproduces it locally. Advisory: it reports, it never gates |
| 4 ✅ | [#1775](https://github.com/zynax-io/zynax/issues/1775) | per-release publication + adapter-author how-to — **done**: every release attaches `zecs-matrix.json` + `zecs-conformance.md` and carries the rendered section in its notes, headed by the one-line claim; the table is rendered from the attached document (`zynax-ci conformance render`), never hand-written, so notes and artifact cannot drift. Each leg publishes with its `enforcement` and both aggregates (`required legs:` / `all legs:`), so an advisory PASS never reads as merge-blocking and #1778 flips it with no template edit. A release may cite only a run whose revision is an ancestor of the released commit; with none available it publishes an all-`NOT_RUN`, `complete: false` document rather than omitting the matrix. How-to: `docs/conformance/how-to-run-zecs.md`; `ci_step` keys now resolve statically against the e2e job's step ids. **EPIC #1692 closed** |
