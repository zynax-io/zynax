...
## Session — 2026-06-25 (post-merge verify — M7.K kind keystone #1508)

### #1508 / #1492 (post-mrg for an infra-only PR)
- For an infra-only PR (Makefile + scripts, no `services/*` and no Dockerfile/images.yaml), the post-merge surface is predictable: CI runs only `dco`/`changes`/`test-unit` (lint/security/build-matrix/test-go skip via path filter), Release runs only the staging->main retag (the version-retag matrix skips), and the GHCR `ci-runner` `latest` digest stays put — so Phase 4b is confirm-only and yields no digest PR. (domain)
- "Push on main" on this repo is the **CodeQL analyze** workflow (go/python/actions), NOT the e2e-smoke lane. The kind `e2e smoke (temporal/argo)` checks run as PR PRE-merge checks, not post-merge main runs — so for an infra/kind PR, post-merge runtime evidence = the green post-merge CI for the SHA confirmed via the Actions API, never a redundant kind re-boot (the delivery already booted twice + PR e2e-smoke covers it). (domain)
- A skipped matrix job shows as the unexpanded `${{ matrix.service }}` template name in `gh run view <run> --json jobs` — that is the expected "no matrix entry selected" signal for a path-filtered infra PR, not an error. (domain)

## Session — 2026-07-10 (merge-queue cutover verification #1685)

### #1685 (ci: merge-queue cutover — ruleset switch + canary verification, M8.I step 5)
- Queue-membership proof lives in the PR issue timeline: `added_to_merge_queue`/`removed_from_merge_queue` events plus merging actor `github-merge-queue[bot]` give the exact arm→queue→merge chain; absence of `head_ref_force_pushed` after `auto_merge_enabled` is the "zero manual rebase" proof. (domain)
- Queue-leg check proof: `gh api "repos/OWNER/REPO/actions/runs?event=merge_group"` — head_branch `gh-readonly-queue/main/pr-<n>-<sha>` with all required workflows success/skipped evidences "all contexts report, no Expected stall" (the M5 failure mode). A required check reported `skipped` on a queue ref satisfies the context (guard-skip design), not a failure. (domain)
